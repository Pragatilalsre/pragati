...
def select_data():
    ...
    if(start_year < end_year):
        ...
        model=input()
        if(model=="ARIMA"):
            ArimaModel(start_date , end_date , n)
        elif(model == "SARIMAX"):
            sarimaxModel(start_date , end_date , n)
        elif(model=="ETS"):
            etsmodel(start_date , end_date , n)
        else:
...
def ArimaModel(start_date , end_date , n):
 ...
 def sarimaxModel(start_date , end_date ,n):
    if(start_date < end_date):
        Sales= Cleaning_data()
        Sales =Sales[start_date:end_date]
        train_size = int(len(Sales) * (1-n))
